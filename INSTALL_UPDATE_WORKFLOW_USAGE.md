...
`make.bat` はリポジトリ直下に置き、Workflowもリポジトリルートから引数なしで実行します。一方、`.exe` の出力場所は固定しません。Workflowはビルド前にリポジトリ配下の `.exe` を記録し、ビルド後に新規・更新された `.exe` を出力場所に関係なく検出します。

- `make.bat` が `install` へ配置した場合は、その更新されたexeを使用します
- `install` 外に配置された場合は、検出した場所から元のファイル名のまま `install` へ移動します
- `.exe` が複数検出されて出力を一意に特定できない場合は、誤った成果物を署名しないため失敗します

このため、`make.bat` が `install` へコピーする構成と、`build` や `bin` など任意の場所へ出力する構成の両方に対応できます。Workflowは`.exe`のファイル名や出力ディレクトリをハードコードしません。
